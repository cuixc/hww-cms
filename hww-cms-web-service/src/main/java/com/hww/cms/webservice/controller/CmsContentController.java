...
public class CmsContentController {
	...
	// ===========================detail=============================
	/**
	 * APP端新闻详情 获取规则：单个新闻id 接口业务：相关新闻列表 + 用户收藏状态 + 用户点赞状态+ 新闻类型 + 文件列表（图片、视频、音频） +
	 * 爆料数 + 爆料列表（最多返回3个） + 评论数 + 评论列表
	 *
	 * @return
	 */
	...
	public R newDetail(HCmsQueryDto cmsQueryDto) {
		...
		CmsContentDataDto cmsContent = cmsContentService.loadNewsDetail(cmsQueryDto);
		log.info("----------APP端新闻详情结束----------");
	...
	public R atlasDetail(HCmsQueryDto cmsQueryDto) {
		...
		try {
			...
			if (cmsQueryDto.getContentId() != null) {
				CmsContentDataDto atlasDetail = cmsContentService.loadNewsDetail(cmsQueryDto);
				log.info("----------图集详情结束----------");
	...
	@ApiOperation(value = "查询专题--内容包含子专题", notes = "")
	@RequestMapping(value = "loadNewsBySpecialId2018.do", method = RequestMethod.POST)
	public R loadNewsBySpecialId2018(HCmsQueryDto cmsQueryDto) {
		log.info("----------loadSpecial----------");
		// 修改为根据前端传过来分页显示
		HCmsSpecialVo cmsSpecialVo = cmsSpecialService.loadCmsSpecialVoById_20180411(cmsQueryDto);
		if (cmsSpecialVo == null) {
			return R.ok().put("data", null);
		}
		log.info("----------loadSpecial---------");
	...
	public R loadContentBySpecialId(HCmsQueryDto cmsQueryDto) {
		...
		//20180411
		if(cmsQueryDto.getPageNo()==null) {//加载更多不分页
			cmsQueryDto.setPageNo(1);
		}
		if(cmsQueryDto.getPageSize()==null) {//加载更多不分页
			cmsQueryDto.setPageSize(200);
		}
	...
	public R loadUserCollectNews(HCmsQueryDto cmsQueryDto) {
		...
		List<CmsContentDto> collectionList = cmsContentService.loadUserCollectNews(cmsQueryDto);
	...
	public List<CmsContentDto> loadUserCollectNewsFeginApi(@RequestBody HCmsQueryDto cmsQueryDto) {
	...
	public R homeHis(HCmsIndexDto cmsIndexDto) {
		...
		// 给数据加上来源信息
		data = cmsContentService.initialOrigin(data);
	...
	/**
	 * 分页查询所有新闻，按时间倒序，分页参数有用，其他参数不起作用
	 * 
	 * @param cmsQueryDto
	 * @return
	 */
	// ===========================feginAPI=============================
	@RequestMapping(value = "listAllCmsContentByPage.do", method = { RequestMethod.GET, RequestMethod.POST })
	public List<CmsContentDto> listAllCmsContentByPage(@RequestBody HCmsQueryDto cmsQueryDto) {
		if (cmsQueryDto.getPageNo() == null)
			cmsQueryDto.setPageNo(1);
		if (cmsQueryDto.getPageSize() == null)
			cmsQueryDto.setPageSize(10);
		return cmsContentService.listAllCmsContentByPage(cmsQueryDto);
	}
	...
	public CmsContentDataDto loadNewsDetailFeginApi(@RequestBody HCmsQueryDto cmsQueryDto) {
	...
	public CmsContentDataDto loadNewsNoPostAndTopicFeginApi(@RequestBody HCmsQueryDto cmsQueryDto) {
		CmsContentDataDto cmsContent = cmsContentService.loadNewsDetailWithoutPostAndTopic(cmsQueryDto);
		return cmsContent;
	}
	...
	public R newsShareDetail(String id) {
		...
		CmsContentDataDto cmsContent = cmsContentService.loadNewsDetailForShare(Long.valueOf(id));
