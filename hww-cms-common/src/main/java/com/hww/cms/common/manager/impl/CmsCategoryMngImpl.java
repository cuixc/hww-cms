...
public class CmsCategoryMngImpl
    ...
    @Override
    public List<AllCategoryTreeDto> getRetrievingFullTree(Long userId, Integer siteId, Short status) {
        List<AllCategoryTreeDto> allCategoryTreeDtos = new ArrayList<AllCategoryTreeDto>();
        List<CmsCategory> cmaCategorys = cmsCategoryDao.getRetrievingFullTree(userId, siteId, status);
        //cmsCategoryDtos = BeanMapper.mapList(cmaCategorys, CmsCategoryDto.class);
        for (CmsCategory category : cmaCategorys) {
            AllCategoryTreeDto allCategoryTreeDto = BeanMapper.map(category, AllCategoryTreeDto.class);
            CmsCategory cmsCategoryParent = category.getParent();
            if (cmsCategoryParent != null) {
                Long parentId = cmsCategoryParent.getCategoryId();
                allCategoryTreeDto.setParentId(parentId);
            }
            allCategoryTreeDtos.add(allCategoryTreeDto);
        }
    ...
    public void saveOrUpdateCategoryAndAndRelationshipWidthTpl(
            CmsCategory entity, CmsCategoryDto form, boolean b) {
        // TODO Auto-generated method stub
