...
public class CmsContentDaoImpl extends LocalDataBaseDaoImpl<Long, CmsContent> implements CmsContentDao {
    ...
    @Override
    public List<CmsContent> listAllCmsContentByPage(Integer pageNo, Integer pageSize) {
        Finder finder=Finder.create("select content_id as contentId,title,short_title as shortTitle,summary,tag_ids as tagIds,keyword_ids as keywordIds, attachment_ids as attachmentIds,create_time as createTime,longitude,latitude,location,thumb_url as thumbUrl,content_type_id as contenttypeId ,audit_step as auditStep ");
        finder.append("from cms_content where content_id in (select content_id FROM (select content_id from cms_content where status=1 and auditStatus=1 order by create_time desc limit ");
        finder.append((pageNo-1)*pageSize+" ,"+pageSize+" ) temp)");
        return (List<CmsContent>) findJoin(finder,CmsContent.class);
    }
}
