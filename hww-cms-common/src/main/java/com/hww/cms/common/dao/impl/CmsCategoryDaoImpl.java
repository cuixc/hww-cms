...
public class CmsCategoryDaoImpl extends LocalDataBaseDaoImpl<Long, CmsCategory> implements CmsCategoryDao {
	...
	@Override
	public CmsCategory getCmsCategoryById(Long id) {
		Finder f = Finder.create("from CmsCategory where categoryId= :categoryId");
		f.setParam("categoryId", id);
		List<CmsCategory> cmsCategories = (List<CmsCategory>) find(f);
		if (cmsCategories != null && cmsCategories.size() > 0)
			return cmsCategories.get(0);
		return null;
	}
}
