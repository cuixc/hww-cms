...
public class CmsLoginController
	...
	public String logout(HttpServletRequest request, HttpServletResponse response) {
		// 注销shiro session
		String strBackUrl = request.getScheme() +"://"+ request.getServerName()+ ":"+ request.getServerPort()  + request.getContextPath() ; 
        
		SecurityUtils.getSubject().logout();
		// 注销tomcat session
		// shiro注销之后再摧毁session，不然会抛异常
		session.logout(request, response);
		return "redirect:"+casConfig.getCasServerLogoutUrl()+"?service="+strBackUrl+"/index.do";
	}
